                ...
                {[
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <span className={`font-medium ${formData[item.key as keyof typeof formData] ? 'text-blue-900 dark:text-white' : 'text-slate-500 dark:text-gray-400'}`}>
                        {item.label}
                      </span>
                    </div>
                    <div className={`w-6 h-6 rounded-full border flex items-center justify-center ${
                      formData[item.key as keyof typeof formData] ? 'bg-blue-500 border-blue-500' : 'border-slate-300 dark:border-gray-600'
                    }`}>
                      {/* REMOVED layoutId to fix multiple checkmark glitch */}
                      {formData[item.key as keyof typeof formData] && <motion.div className="w-2 h-2 bg-white rounded-full" />}
                    </div>
                  </button>
                ))}
              </motion.div>
